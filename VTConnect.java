...
//    }
    ...
    /** Dictionary of (key, vertex) pairs*/
    private Graph<Integer> grph;
    /** Dictionary of (key, profile) pairs */
    ...
    public VTConnect() { 
        ...
        profileDictionary = 
            new HashMap<Integer,Profile>(101);
    }
    ...
    public void addUser(Profile p) {
        ...
        List<Profile> profileList = 
            new ArrayList<Profile> (profileDictionary.values());
        ...
        for (Integer i=0; i< profileList.size(); i++) {
            ...
            if (currentProfile.toString().
                compareTo(p.toString())==0)
            {
                return;
            }
        }
...
successfully, it returns true, false otherwise. 
    ...
    public boolean createFriendship(Profile a, Profile b) {
        ...
        if (!profileDictionary.containsKey(a.id) 
            || !profileDictionary.containsKey(b.id)
            || a.id == b.id){
            return false;
        }
...
profile. 
     * @param startPoint start of traversal
     */
    public void traverse(Profile startPoint) {
        Queue<Integer> traversalOrder = 
            grph.getBreadthFirstTraversal(startPoint.id);
...
given profile exists in VTConnect, false otherwise. 
     * @param user user profile to query
...
does not exist or if it does not have any friend suggestions.
    ...
    public List<Profile> friendSuggestion(Profile user) { 
        ...
        VertexInterface<Integer> originVertex = 
            grph.vertices.get(user.id);
        originVertex.visit();
        Iterator<VertexInterface<Integer>> itrNeigh = 
            originVertex.getNeighborIterator();
        Iterator<VertexInterface<Integer>> itrNeigh2;
        List<Profile> suggestionList = 
...
distance between two profiles.
    ...
    private void resetVertices() { 
        List<VertexInterface<Integer>> vertexList = 
            grph.getVertices();
